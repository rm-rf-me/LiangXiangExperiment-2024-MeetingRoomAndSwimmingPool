...
def plot_data_list_with_wave_height(data_list, title, start_time, save_path=None):
    ...
    for k, data in data_list.items():
        if "Time" in data:
            ...
            ax2.plot(data["TimeDelta"][1:], data["Value"][1:] - data["Value"][1:].mean(),
                    color=wave_color, 
                    label='Wave Height',
                    linewidth=1.5)
        else:
            ...
            ax1.plot(df['TimeDelta'], data[4][1:]/2,
                    color=signal_color, 
